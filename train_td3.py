...
def main_train():
    ...
    # save models
    torch.save(agent.actor.state_dict(), model + f"td3_actor_reach.pth")
    torch.save(agent.critic.state_dict(), model + f"td3_critic_reach.pth")
    np.save("episode_rewards_td3.npy", np.array(reward_history))

    env.close()

    # plot the training reward history
    plot_reward(reward_history, window=20, model_name="TD3")
